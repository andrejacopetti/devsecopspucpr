pipeline {
    ...
    environment {
        DOCKER_REPO = 'andrejacopetti/projeto-ecommerce'
        DOCKER_IMAGE_APP = "${DOCKER_REPO}:latest"
        DOCKER_IMAGE_DB = "andrejacopetti/projeto-ecommerce-db:latest"
        K8S_DIR = 'k8s/'
        SQL_DIR = 'sql/'
        ...
        stage('Snyk Docker Image Scan - App') {
            steps {
                sh '''
                    ...
                    snyk container test ${DOCKER_IMAGE_APP} --severity-threshold=medium --file=/var/www/html/devsecopspucpr/Dockerfile --exclude-base-image-vulns
                '''
            }
        }
